...
최건호: 맵리듀스에서 output이 utf-8코드로 나오는 현상 Rawprotocol로 조정가능 하지만 count하는 숫자까지 인코딩을 하려고 해서 이부분만 조정하면될거같음\
 출처: https://github.com/Yelp/mrjob/issues/1442 \
 문제 해결 해당 오류는 ISSUE란에서 코드 확인: https://github.com/BDP-Term-Project/Term-Project/issues/3
 
**2023-12-08**\
맵리듀스 한글 인코딩 문제 해결 후 **네이버 17개 지역 별 크롤링 데이터(ex. 강원) + 네이버 세부 지역 검색 후(ex. 강원_강릉시) 17개 지역 별로 묶은 데이터 + 인스타그램 17개 지역 별 크롤링 데이터**를 17개 지역 별로 합성하여 최종 크롤링 데이터 완성\
 -> twitter 라이브러리를 사용해 최종 데이터에서 불용어 제거 및 명사 추출\
 -> 추출한 명사를 바탕으로 맵리듀스 워드 카운팅(내림차순 정렬)\
 -> 워드클라우드를 이용해 워드 카운팅 txt를 시각화
